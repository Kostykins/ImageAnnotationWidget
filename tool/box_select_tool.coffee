...
define [
  ...
  class BoxSelectToolView extends Tool.View
    ...
    _clear_selection: (e) ->
      ...
      [x,y] = @_get_mouse_pos(e)
      @trigger('grabboxarray', x,y, [x,y])
      if x >= @xrange[0] and x <= @xrange[1] and y >= @yrange[0] and y <= @yrange[1]
        #save the current selection as an object.
        @trigger('saveselect', @xrange, @yrange, [x,y])
      else
        @trigger('stopselect')
        @plot_view._render_levels(@plot_view.canvas_view.ctx, ['overlay'])
        for renderer in @mget('renderers')
